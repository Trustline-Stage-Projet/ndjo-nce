...
@Component({
  selector: 'app-update-chauffeurs',
  templateUrl: './update-chauffeurs.component.html',
  styleUrls: ['./update-chauffeurs.component.css'],
})
export class UpdateChauffeursComponent implements OnInit {
  userInfo: any;
  id: any;
  ...
  getData() {
    ...
    this._dataService.getChauffeurDetails(this.id).subscribe(
      (data) => {
    ...
    setTimeout(() => {
      this._dataService.PersonnalUpdateChauffeur(this.chauffeurId, formData).subscribe(
        ...
        (error) => {
          ...
          console.error(error)
          this._modalService.open(content, { centered: true });
          this.errorMessage='Une erreur est survenu lors de la mise a jours du chauffeur '
        }
      );
    }, 1000);
  }
  ...
  delete(content:any): void {
    if (this.id) {
      this._dataService.deleteChauffeur(this.id).subscribe({
        next: (res) => {
          console.log(res, 'Chauffeur supprimer avec succes');

          this._modalService.open(content, { centered: true });
          this.successMessage = 'Chauffeur supprimer avec succes'
        },
        error: (e) => console.error(e),
      });
    } else {
      console.error('Impossible de supprimer le chauffeur : ID non valide');
      this._modalService.open(content, { centered: true });
      this.errorMessage = 'une erreur est surveunu lors de la supression du compte du chauffeur'
    }
  }

  openConfirmationModal(content: any,content2:any) {
    this._modalService
